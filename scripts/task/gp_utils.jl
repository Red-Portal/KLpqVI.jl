...
se_kernel(α², logℓ) =
    α²*(KernelFunctions.SEKernel() ∘ KernelFunctions.ARDTransform(@. exp(-logℓ)))
    ...
    if issuccess(K_chol)
        ...
        function (Δ)
            ...
            ∂p∂ℓ² = map(1:n_dims) do k
                #∂K∂ℓ²ᵢ     = gram_se_derivative_gpu(K, R, X_dev, Int32(k), σ², ℓ²_dev)
...
function marginal_variance(K, W_factor, K_test_train)
    W_factor*B_chol
end
