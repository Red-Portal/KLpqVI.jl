...
%%     \textit{Weight} \hspace{0.01in} \(\tilde{w}^{(i)} = p(\vz^{(i)},\vx) \,/\, q_{\vlambda_{t-1}}(\vz^{(i)}) \) for \(i = 0, 1,\ldots, N\)\;
%%     \textit{Normalize} \(w^{(i)} = \nicefrac{\tilde{w}^{(i)}}{ \sum^{N}_{i=0} \tilde{w}^{(i)} }\) for \(i = 0, 1,\ldots, N\)\;
%%     \(\vz_{t} \sim \mathrm{Multinomial}(w_{t-1}, w^{(1)}, \ldots, w^{(N)}) \)\;
%%     \caption{Conditional Importance Sampling}
%%   \end{algorithm2e}
%% \end{minipage}
