...
Bayesian inference aims to analyze the posterior distribution of an unknown latent variable \(\vz\) from which the data \(\vx\) is observed \jrg{I don't know that I'd call these latent variables in the most general setting. Historically, these were more commonly parameters of a model $\theta$. I also think $\vz$ here clashes with the use of $\vz$ later to mean the samples from the Markov chain}.
...
\begin{enumerate}
    ...
    \item To accomplish this, we show that both methods can be viewed as what we call ``Markov chain score ascent'' (MCSA) methods, which are a specical case of the MCGD framework. \jrg{Note we want to avoid words like ``automatic'' -- it makes the contribution sound trivial.}
