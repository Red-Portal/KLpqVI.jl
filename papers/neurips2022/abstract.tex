...
\krk{
  Minimizing the inclusive Kullback-Leibler (KL) divergence with stochastic gradient descent (SGD) is challenging due to the fact that it requires gradients averaged over the posterior.
  However, multiple methods have recently emerged showing that it is possible to run SGD with \textit{biased} gradient estimates obtained from Markov chains,
}
