...
Apart from properties of the objective function (such as Lipschitz smoothness), the convergence rates are stated in terms of the gradient bound \(G\), kernel mixing rate \(\rho\), and number of MCGD iteration \(T\).
...
This observation is critical since it implies that trading gradient variance and mixing rate could be profitable.
...
\end{enumerate*}
This will enable theoretical analysis of their non-asymptotic convergence.
...
It does help escaping slow mixing regions in the initial steps, but this benefit vanishes quickly as we converge.
However, boundedness is necessary for ensuring~\cref{thm:kernel_conditions}.
Although previous MCSA methods did not take specific measures to ensure boundedness, this can be done by using a variational family with heavy tails~\citep{NEURIPS2018_25db67c5} or using a defensive mixture~\citep{hesterberg_weighted_1995, holden_adaptive_2009} as
\begin{align*}
...
We empirically justify our bounds obtained using~\cref{thm:bounded_score} in \cref{section:simulation}.
...
Overall, the performance of MSC cannot be improved by increasing the computational budget \(N\).

\vspace{-0.1in}
\subsection{Joint Stochastic Approximation}
\vspace{-0.07in}
JSA~\citep{pmlr-v124-ou20a} was originally proposed for deep generative models where the likelihood factorizes into each datapoint, in which subsampling can be done by using a random-scan version of the independent Metropolis-Hastings (IMH, \citealt{hastings_monte_1970}) kernel.
Instead, we consider the general version of JSA with the vanilla IMH kernel which can be used with non-factorizable likelihoods.
At each MCGD step, JSA performs multiple Markov chain transitions and estimates the gradient by averaging all the intermediate states.
...
The results of~\cref{section:comparison} suggest that the performance of both MSC and JSA worsens as \(w^*\) becomes large.
Furthermore, this negative impact cannot be easily counterbalanced by increasing the computational budget \(N\).
...
Therefore, we propose a novel scheme, \textit{parallel Markov chain score ascent} (pMCSA), that embraces a slower mixing rate in order to consistently achieve a \(\mathcal{O}\left(\nicefrac{1}{N}\right)\) variance reduction.
...
Assuming caching is done as much as possible, pMCSA needs twice more evaluations of \(q\,(\vz; \vlambda)\).
