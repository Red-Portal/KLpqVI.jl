...
\vspace{-0.1in}
\paragraph{\textbf{Model (Variational Family) Misspecification and \(\mathbf{w^*}\)}}
...
\vspace{-0.07in}
We propose a novel scheme, \textit{parallel Markov chain score ascent} (pMCSA,~\cref{alg:pmcsa}), that embraces a slower mixing rate in order to consistently achieve a \(\mathcal{O}\left(\nicefrac{1}{N}\right)\) variance reduction, even on challenging problems with in a large \(w^*\),  
