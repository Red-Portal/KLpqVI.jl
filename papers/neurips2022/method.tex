...
\textcolor{blue}{
Now, the variance decreases as \(\mathcal{O}\left(\nicefrac{1}{N-1}\right)\), but the first term depends on the \(\chi^2\) divergence, which is exponentially bigger than the KL divergence~\citep{10.1214/17-STS611}, and the second term depends quadratically on \(w^*\).
Therefore, the variance of MSC-RB will be large on challenging problems such that \(w^*\) and the \(\chi^2\) divergence are large.
}
...
\vspace{-1ex}
\paragraph{Discussion}
As shown in~\cref{thm:jsa} JSA benefits from increasing \(N\) in terms of faster mixing rate.
\textcolor{blue}{
However, for the variance, under lack of convergence and model misspecification (large \(w^*\)), the variance improvement becomes marginal.
More importantly, \textit{in the large \(w^*\) regime, the variance reduction is limited by the constant \(1/2\) term}.
This is true even if we completely ignore the covariance between the samples \(C_{\text{cov}}\) which, in practice, will only increase variance.
}
