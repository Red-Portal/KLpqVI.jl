...
We modified their proof to match our desired result.
}

\input{thm_previous_mscrb}

\textcolor{blue}{
In this case, the variance does improve with \(N\), but the first term depends on the \(\chi^2\) divergence, which is bounded below exponentially by the KL divergence~\citep{10.1214/17-STS611}.
This means that its variance will be large on challenging problems.
Note that, if we bound the actual variance instead of just the second moment, one can obtain a tighter {\small\(\mathcal{O}\left({\left(N-1\right)}^{-1}\right)\)} bound~\citep{cardoso_brsnis_2022} (the second term can be made to decrease in an {\(\mathcal{O}\left(N^{-2/3}\right)\)} rate).
However, the linear dependence on the \(\chi^2\) divergence still persists.
}
