...
Given some observed data \(\vx\), the goal is to analyze an unobserved variable \(\vz\) by assuming a model \(p\,(\vx\mid\vx)\), and a prior distribution \(p(\vz)\).
The distribution of \(\vz\) is given by Bayes' rule such that \(p\,(\vz\mid\vx) \propto p\,(\vx|\vz)\,p\,(\vz) = p\,(\vz,\,\vx)\).

Instead of working directly with the target distribution \(p\,(\vz\mid\vx)\), \textit{variational approximation} \(q_{\lambda}(\vz) \in \mathcal{Q}\) approximate \(p\,(\vz\mid\vx)\) according to a discrepancy measure \(D\,(p, q)\).
...
We show that \textit{independent Metropolis-Hastings} (IMH,~\citealt{robert_monte_2004}) type kernels can automatically trade off bias and variance when used for MSC.
