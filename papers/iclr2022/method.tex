...
%\vspace{-0.05in}
\section{Chain Wielding Strategies for Markov-chain Monte Carlo Based Inclusive Variational Inference}
%\vspace{-0.05in}
\subsection{Overview of Prevoius Estimation Strategies}\label{section:msc}
%
%\vspace{-0.1in}
...
For example, in their method named Markovian score climbing (MSC), \citep{NEURIPS2020_b2070693} estimate the score gradient by performing an MCMC iteration and update the parameters such that
\begin{align}
  &\vz_t \sim K\left(\vz_{t-1}, \cdot \right) &g_{\mathrm{single}}(\vlambda) = s\,(\vz_t; \vlambda)
\end{align}
...
We will later call this estimator the ``single mode'' estimator.

\paragraph{Joint Stochastic Approximation}
On the other hand, at each SGD iteration \(t\),~\citep{pmlr-v124-ou20a} perform \(N\) sequential Markov-chain transitions and use the average of the intermediate states for estimation such that
\begin{align}
  &\vz_{T+i} \sim K^i\left(\vz_{T}, \cdot \right) &g_{\mathrm{seq.}}(\vlambda) = \frac{1}{N} \sum_{i=1}^N s\,(\vz_{T+i}; \vlambda)
\end{align}
where \(\vz_T\) is the last Markov-chain state of the previous SGD iteration.
We will later call this estimator the ``sequential mode'' estimator, since the Markov-chain states used for estimation are formed sequentially.

\paragraph{Additional  Notes on JSA}
Although their setting is slightly different since their MCMC kernel leave \(p\left(\vz_j\mid\vx_j \right)\) invariant for a single datapoint \(\vx_j\), this is only possible when the datapoints are independently, identically distributed (\textit{iid}).
To simplify our analysis, we interpret their method more generally and assume that we only have a kernel that can leave \(p\left(\vz\mid\vx \right)\) invariant.

\paragraph{MCMC Kernels of JSA and MSC}
For the MCMC kernel, \citeauthor{NEURIPS2020_b2070693} specifically propose to use the conditional importance sampling (CIS) kernel.
In contrast,~\citeauthor{pmlr-v124-ou20a} use the independent Metropolis-Hastings (IMH,~\citealt[Algorithm 25]{robert_monte_2004}).
Interestingly, the computational cost of JSA is comparable to MSC because of the additional cost of the CIS kernel.
...
\subsection{Overview of Markov-Chain Monte Carlo Estimation Schemes}\label{section:msc_mcmc}
...
\paragraph{Single Mode and Sequential Mode}
The two different MCMC-based score estimation schemes of MSC and JSA represent two different ways of using an MCMC kernel.
The former uses a computationally expensive, but hopefully statistically superior, MCMC kernel.
The latter uses a cheaper MCMC kernel, but generates more samples for estimation.
Since the scheme of MSC uses only a single sample for estimation, we call it the ``single mode'' estimator, \(g_{\mathrm{single}}\).
Meanwhile, since the scheme of JSA uses multiple sequential Markov-chain states for estimation, we call it the ``sequential mode'' estimator, \(g_{\text{seq.}}\)

\paragraph{Parallel Mode}
While the sequential mode estimator is a 
