...
%\vspace{-0.05in}
\section{Estimation Strategies for Markov-chain Monte Carlo Copuled Inclusive Variational Inference}
%\vspace{-0.05in}
...
For example, in their method named Markovian score climbing (MSC), \citeauthor{NEURIPS2020_b2070693} estimate the score gradient by performing an MCMC iteration and update the parameters such that
\begin{align}
...
On the other hand, at each SGD iteration \(t\),~\citeauthor{pmlr-v124-ou20a} perform \(N\) sequential Markov-chain transitions and use the average of the intermediate states for estimation such that
\begin{align}
