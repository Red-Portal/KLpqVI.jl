...
Our proposed parallel state estimator (par.-IMH) achieves the best results both in terms of accuracy and predictive density.
...
While both ELBO and par.-IMH showed similar numerical performance, in the parameter space, they choose different optimization paths.
...
Compared to ELBO, its accuracy was lower on \texttt{sonar} but the uncertainty estimates where much better.
...
This happens when the modal estimate (preferred by the exclusive KL) gives both good accuracy and uncertainty estimates.
...
By contrast, other estimator converge much slowly.
