...
We choose the target posterior to be a 10 dimensional white Gaussian.
We then randomly generate 2048 random \(\vlambda = \vmu\) where \(q_{\vlambda}(\vz) = \mathcal{N}(\vmu, \mSigma)\), \(\vmu\) is drawn from a multivariate Student's T distribution, and \(\mSigma = 1.5^2 \mI\).
Using the 2048 random \(\vlambda\)s, we simulate 128 Markov-chains of length \(T=50\) and estimate the bias and variance of the score function.
...
The computational budget is set to \(N=10\) and the number of SGD iterations is \(T=10^4\) for all experiments unless specified.
...
\vspace{-0.05in}
\paragraph{Results}
The test accuracy and test log predictive density (LPD) results are shown in~\cref{table:logistic}.
...
This supports our analysis that par.-IMH with \(N > 1\) has superior variance reduction to the single state estimator.
Also, seq.-IMH showed poor performance overall due to the correlation of the samples.
...
\begin{figure}[H]
  ...
  \subfloat[Test Accuracy]{
    \includegraphics[scale=0.90]{figures/german_02.pdf}\label{fig:german_acc}
  }  \\
  ...
  \subfloat[Test Log Predictive Density]{
    \includegraphics[scale=0.90]{figures/german_03.pdf}\label{fig:german_lpd}
  }
    \vspace{-0.05in}
...
Again, among inclusive KL minimization, the parallel state estimator (par.-IMH) achieves the best results.
Compared to ELBO, its accuracy was lower on \texttt{sonar}, but the uncertainty estimates are much better.
...
We estimated the reference marginal likelihood using thermodynamic integration (TI,~\citealt{gelman_simulating_1998, neal_annealed_2001, lartillot_computing_2006}) with HMC implemented by Stan~\citep{carpenter_stan_2017, betancourt_conceptual_2017}.
%
\begin{figure}[H]
  \vspace{-0.15in}
  \centering
