...
\paragraph{CIS has high rejection rate until MSC converges.}
...
\begin{minipage}[t]{0.45\textwidth}
  ...
  \begin{algorithm2e}[H]
    ...
    \For{\textcolor{black}{\(t = 1, 2, \ldots, T\)}}{
      \For{\textcolor{black}{\(i = 1, 2, \ldots, N\)}}{
        \(\vz^{(i)}_{t} \sim K(\vz^{(i)}_{t-1}, \cdot)\)
      }
      \( s(\vz^{(i)}_t; \vlambda) = \nabla_{\vlambda} \log q_{\vlambda}(\vz_t^{(i)}) \)\;
      \( \vlambda_{t} = \vlambda_{t-1} + \gamma_t\, \frac{1}{N} \sum^{N}_{i=1} s\,(\vz_t^{(i)};\vlambda_{t-1}) \)\;
    }
