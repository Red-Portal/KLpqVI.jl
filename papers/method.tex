...
This \textit{marginal} estimate of \(f\) is given as
\begin{align}
...
\item The usual Central Limit Theorem guarantees of MCMC do not apply.
...
\begin{align}
  ...
  &=\Vsub{q_{\vlambda}}{ \E{ f \mid \vz_{t-1},\,\vz^{(1:N)} } } + \underbrace{\Esub{q_{\vlambda}(\vz^{(1:N)})}{ \V{ f \mid \vz_{t-1},\,\vz^{(1:N)} } }}_{\text{Rao-Blackwellization gain}} \label{eq:total_variance} \\
...
Note that these properties hold similarly in any IMH type kernel.
...
In the small \(N\) regime, PIMH has a slightly larger conditional variance, which can be easily fixed by using Barker's ratio.
